...
class PatternGenerator {
    ...
    keyGenerator(keyword, isWordFormats, isDateFormats, isCaseSenitive) {
        ...
        if (isWordFormats) { //?! handle abc123 case..it shld be may be covered in other case check...
            ...
            if (regexExp.digits.test(keyword)) { // all number dates
                ...
                pattern = this.monthsEquation(keyword, isCaseSenitive);
            }
        } else {
    ...
    monthsEquation(keyword, isCaseSenitive) {
        let finalEq = '';
        let changeNums = '';
        let spclKey = keyword.replace(regexExp.specialChars, 'spcl');
        let changeMonths = spclKey.replace(/(?:Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|(Nov|Dec)(?:ember)?)/ig, `${regexExp.monthsEq}`);
    ...
    getRegexMethodEq(regexEq, method, keyword, isCase, isGlobal, isReplace) {
        let eq = '';
        console.log("🚀 ~ file: pattern-generator.js ~ line 96 ~ PatternGenerator ~ getRegexMethodEq ~ isCase", isCase, isGlobal, keyword, regexEq)
    ...
    regexEquation(inputData, isCase, isGlobaly, isWords) {
        ...
        if (typeof (inputkeys) != 'string') {
            inputkeys.forEach((keyword) => {
                formedRegex = this.keyGenerator(keyword, isWordFormats, isDateFormats, isCaseSenitive);
            ...
            formedRegex = this.keyGenerator(inputkeys, isWordFormats, isDateFormats, isCaseSenitive);
