...
class PatternGenerator {
    ...
    formPattern(keyword, isWordFormats, isAlphaNumForm, isDateFormats, isCaseInSenitive) {
        console.log("🚀 ~ file: pattern-generator.js ~ line 6 ~ PatternGenerator ~ formPattern ~ keyword, isWordFormats, isDateFormats, isCaseInSenitive", keyword, isWordFormats, isAlphaNumForm, isDateFormats, isCaseInSenitive);
        try {
            let pattern;
            ...
            } else if (isAlphaNumForm) {
                pattern = this.handleAlphaNumForm(keyword);
            } else {
    ...
    getRegexExpression(inputData, pattern, isCase = false, isGlobaly = false) {
        try {
            ...
            if (regexExp.words.test(pattern)) {
                ...
                isWordFormats = false;
                isDateFormats = false;
            } else if (/alphaNumeric/i.test(pattern)) {
                isAlphaNumForm = true;
            }
            ...
            if (typeof (inputkeys) != 'string') {
                inputkeys.forEach((keyword) => {
                    formedRegex = this.formPattern(keyword, isWordFormats, isAlphaNumForm, isDateFormats, isCaseInSenitive);
                    multiplePatterns.push(formedRegex);
                });
                let expression = this.regexFormater(multiplePatterns, isCaseInSenitive, isGlobal);
                return expression;
            } else {
                console.log('entered in one case', inputkeys, isWordFormats, isAlphaNumForm, isDateFormats, isCaseInSenitive);
                let filteredInput = inputkeys.replace(/^[\"\']/, '').replace(/[\"\']$/, '');
                formedRegex = this.formPattern(filteredInput, isWordFormats, isAlphaNumForm, isDateFormats, isCaseInSenitive);
