...
class PatternGenerator {
    ...
    getRegexMethodEq(regexEq, method, keyword, isCase, isGlobal, isReplace) {
        try {
            ...
            console.log("🚀 ~ file: pattern-generator.js ~ line 131 ~ PatternGenerator ~ getRegexMethodEq ~ result", result);
            return {
                keyword,
                result
            };
    ...
    getRegexExpression(inputData, pattern, isCase = false, isGlobaly = false) {
        try {
            ...
            let isCaseInSenitive = isCase ? 'i' : '';
            let isGlobal = isGlobaly ? 'g' : '';
            console.log("🚀 ~ file: pattern-generator.js ~ 198 ~ PatternGenerator ~ getRegexExpression ~ isGlobal", isGlobal);
