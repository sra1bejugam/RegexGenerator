        ...
        try {
            ...
            if (isWordFormats) { // check if keywords is digit then shld fail on Ui saying valid options...
                ...
                if (/^[a-z]+$/i.test(keyword)) { // only alphabets
                    pattern = keyword;
                } else if (/^(?=.*[a-z])(?=.*[0-9])[a-z0-9-.\/!@#$%^&*\(\).~\`\\s]+$/i.test(keyword)) { // as of 2018 - as[\W]+of[\W+][0-9]{4}
                    let count = keyword.match(/\d+/g);
                    // loop here for multiple count cases jan 20 2619 N cases
                    if (count && count.length === 1) {
                        pattern = alphas.replace(count[0], '[0-9]{' + count[0].length + '}');
                    } else if (count.length > 1) {
                        pattern = alphas.replace(count[0], '[0-9]{' + count[0].length + '}').replace(count[1], '[0-9]{' + count[1].length + '}');
                    }
                }
                if (spclCount && spclCount.length === 1) {
                    pattern = pattern.replace(regexExp.specialChars, '\\W');
                } else if (spclCount && spclCount.length > 1) {
                    pattern = pattern.replace(regexExp.specialChars, '\\W+');
                }
