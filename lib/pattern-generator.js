...
class PatternGenerator {
    ...
    regexEquation(inputData, pattern, isCase = false, isGlobaly = false) {
        try {
            ...
            let isCaseInSenitive = isCase ? 'i' : '';
            let isGlobal = isGlobaly ? 'g' : '';
            console.log("🚀 ~ file: pattern-generator.js ~ 198 ~ PatternGenerator ~ regexEquation ~ isGlobal", isGlobal);
            ...
            if (typeof (inputkeys) != 'string') {
                ...
                formedRegex = this.formPattern(inputkeys, isWordFormats, isDateFormats, isCaseInSenitive);
                const res = formedRegex ? [formedRegex] : undefined;
                return this.regexFormater(res, isCaseInSenitive, isGlobal);
            }
        } catch (err) {
            console.log("🚀 ~ file: pattern-generator.js ~ catch ~ PatternGenerator ~ regexEquation ~ err", err);
        }
    }
}
